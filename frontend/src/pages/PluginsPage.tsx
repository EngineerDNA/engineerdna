...
export function PluginsPage() {
  ...
  const totalPages = Math.ceil(filteredAndSortedPlugins.length / pageSize);
  const paginatedPlugins = filteredAndSortedPlugins.slice((page - 1) * pageSize, page * pageSize);
  ...
  if (isLoading) {
    return (
      <div>
        <h2 className="text-2xl font-bold mb-6 text-gray-900 dark:text-gray-100">Plugins</h2>
        <LoadingSkeleton />
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Plugins</h2>
        <p className="text-gray-600 dark:text-gray-400 mt-1">
          Manage data sources, destinations, and processors
        </p>
      </div>
      ...
      <div className="mb-6 space-y-4">
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <select
            value={typeFilter}
            onChange={(e) => {
              setTypeFilter(e.target.value as PluginType);
              setPage(1);
            }}
            className="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          >
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="text-sm text-gray-600 dark:text-gray-400">
            Showing {paginatedPlugins.length === 0 ? 0 : (page - 1) * pageSize + 1}-
            {Math.min(page * pageSize, filteredAndSortedPlugins.length)} of{' '}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
            {paginatedPlugins.map((plugin) => (
              <PluginCard
                ...
                onSchedule={
                  plugin.type === 'destination' ? () => setSchedulingPlugin(plugin.name) : undefined
                }
              />
            ))}
          </div>
