...
// Cost Configuration operations

// CreateCostConfiguration creates a new cost configuration
func (s *CostROIStore) CreateCostConfiguration(config *models.CostConfiguration) error {
	if config.ID == "" {
	...
	_, err := s.db.Exec(`
		INSERT INTO cost_configuration (
			id, entity_type, entity_id, role, monthly_cost, currency,
			effective_from, effective_to, notes, created_at, updated_at
		) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`,
		config.ID, config.EntityType, config.EntityID, config.Role,
		config.MonthlyCost, config.Currency, config.EffectiveFrom.UTC(),
		config.EffectiveTo, config.Notes, config.CreatedAt, config.UpdatedAt,
	)

	if err != nil {
		return fmt.Errorf("failed to create cost configuration: %w", err)
	}

	return nil
}

// GetCostConfiguration retrieves cost configuration by ID
func (s *CostROIStore) GetCostConfiguration(id string) (*models.CostConfiguration, error) {
	var config models.CostConfiguration
	var entityID, role, notes sql.NullString
	var effectiveTo sql.NullTime

	err := s.db.QueryRow(`
		SELECT id, entity_type, entity_id, role, monthly_cost, currency,
		       effective_from, effective_to, notes, created_at, updated_at
		FROM cost_configuration
		WHERE id = ?
	`, id).Scan(
		&config.ID, &config.EntityType, &entityID, &role,
		&config.MonthlyCost, &config.Currency, &config.EffectiveFrom,
		&effectiveTo, &notes, &config.CreatedAt, &config.UpdatedAt,
	)
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get cost configuration: %w", err)
	}
	...
	if effectiveTo.Valid {
		config.EffectiveTo = &effectiveTo.Time
	}
	if notes.Valid {
...
// GetCurrentCostForEntity retrieves current cost for entity
func (s *CostROIStore) GetCurrentCostForEntity(entityType, entityID string) (*models.CostConfiguration, error) {
	var config models.CostConfiguration
	var entID, role, notes sql.NullString
	var effectiveTo sql.NullTime
	...
	query := `
		...
		ORDER BY entity_id DESC NULLS LAST, effective_from DESC
		LIMIT 1
	`

	now := time.Now().UTC()
	err := s.db.QueryRow(query, entityType, entityID, now, now).Scan(
		&config.ID, &config.EntityType, &entID, &role,
		&config.MonthlyCost, &config.Currency, &config.EffectiveFrom,
		&effectiveTo, &notes, &config.CreatedAt, &config.UpdatedAt,
	)
...
// ListCostConfigurations retrieves all cost configurations
func (s *CostROIStore) ListCostConfigurations(entityType string, limit, offset int) ([]*models.CostConfiguration, int, error) {
	...
	countQuery := "SELECT COUNT(*) FROM cost_configuration" + whereClause
	var total int
	err := s.db.QueryRow(countQuery, args...).Scan(&total)
	if err != nil {
		return nil, 0, fmt.Errorf("failed to count cost configurations: %w", err)
	}
	...
	query := `
		SELECT id, entity_type, entity_id, role, monthly_cost, currency,
		       effective_from, effective_to, notes, created_at, updated_at
		FROM cost_configuration` + whereClause + `
	...
	if err != nil {
		return nil, 0, fmt.Errorf("failed to list cost configurations: %w", err)
	}
	defer rows.Close()

	var configs []*models.CostConfiguration
	for rows.Next() {
		var config models.CostConfiguration
		var entityID, role, notes sql.NullString
		var effectiveTo sql.NullTime
		...
		if err != nil {
			return nil, 0, fmt.Errorf("failed to scan cost configuration: %w", err)
		}
...
// Cost Efficiency Metrics operations

// CreateCostEfficiencyMetric creates a cost efficiency metric
func (s *CostROIStore) CreateCostEfficiencyMetric(metric *models.CostEfficiencyMetric) error {
	if metric.ID == "" {
		metric.ID = uuid.New().String()
	}
	if metric.ComputedAt.IsZero() {
		metric.ComputedAt = time.Now().UTC()
	}

	_, err := s.db.Exec(`
		INSERT INTO cost_efficiency_metrics (
			id, time_period, entity_type, entity_id, metric_name,
			metric_value, currency, computed_at
		) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
	`,
		metric.ID, metric.TimePeriod, metric.EntityType, metric.EntityID,
		metric.MetricName, metric.MetricValue, metric.Currency, metric.ComputedAt,
	)

	if err != nil {
		return fmt.Errorf("failed to create cost efficiency metric: %w", err)
	}

	return nil
}

// GetCostEfficiencyMetrics retrieves efficiency metrics
func (s *CostROIStore) GetCostEfficiencyMetrics(timePeriod, entityType string, entityID *string, limit int) ([]*models.CostEfficiencyMetric, error) {
	...
	query := `
		SELECT id, time_period, entity_type, entity_id, metric_name,
		       metric_value, currency, computed_at
		FROM cost_efficiency_metrics
		WHERE time_period = ? AND entity_type = ?
	`
	args := []interface{}{timePeriod, entityType}
	...
	query += " ORDER BY metric_name LIMIT ?"
	args = append(args, limit)

	rows, err := s.db.Query(query, args...)
	if err != nil {
		return nil, fmt.Errorf("failed to get cost efficiency metrics: %w", err)
	}
	defer rows.Close()

	var metrics []*models.CostEfficiencyMetric
	for rows.Next() {
		var metric models.CostEfficiencyMetric
		var entityID sql.NullString

		err := rows.Scan(
			&metric.ID, &metric.TimePeriod, &metric.EntityType, &entityID,
			&metric.MetricName, &metric.MetricValue, &metric.Currency,
			&metric.ComputedAt,
		)
		if err != nil {
			return nil, fmt.Errorf("failed to scan cost efficiency metric: %w", err)
		}

		if entityID.Valid {
			metric.EntityID = &entityID.String
		}
